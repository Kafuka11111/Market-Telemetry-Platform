...
#property indicator_label1  "Open Interest"
#property indicator_type1   DRAW_LINE
#property indicator_color1  C'156,39,176'
#property indicator_style1  STYLE_SOLID
#property indicator_width1  2

double BufferOI[];
string current_symbol = "";
struct TCsvPoint { datetime time; double oi; };
TCsvPoint csvData[];
...
int OnInit() {
   ...
   SetIndexBuffer(0, BufferOI, INDICATOR_DATA); IndicatorSetString(INDICATOR_SHORTNAME, "Global Open Interest (Mln $)"); IndicatorSetInteger(INDICATOR_DIGITS, 2);
...
void LoadCSV() {
   ...
   while(!FileIsEnding(handle)) {
      ...
      if(ArraySize(cols) >= 7) { 
         ...
         csvData[idx].oi = StringToDouble(cols[6]) / 1000000.0;
      }
   }
   FileClose(handle);
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[]) {
   ...
   for(int i = limit; i < rates_total; i++) {
      ...
      if (t_end <= csvData[0].time) { BufferOI[i] = 0; continue; }
      ...
      if (end_idx != -1) {
         BufferOI[i] = csvData[end_idx].oi;
      } else if (i > 0) { 
         BufferOI[i] = BufferOI[i-1]; 
      }
   }
   return(rates_total);
}