...
//+------------------------------------------------------------------+
#property copyright "Quantum Radar DEMO"
...
input int      InpUtcOffset = 0; // Broker UTC Offset 
...
void DrawWatermark() {
   string wmName = "QPRO_WATERMARK";
   if(ObjectFind(0, wmName) < 0) {
      ObjectCreate(0, wmName, OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, wmName, OBJPROP_CORNER, CORNER_RIGHT_LOWER);
      ObjectSetInteger(0, wmName, OBJPROP_XDISTANCE, 20);
      ObjectSetInteger(0, wmName, OBJPROP_YDISTANCE, 20);
      ObjectSetString(0, wmName, OBJPROP_TEXT, "qtp.life");
      ObjectSetString(0, wmName, OBJPROP_FONT, "Trebuchet MS");
      ObjectSetInteger(0, wmName, OBJPROP_FONTSIZE, 35);
      ObjectSetInteger(0, wmName, OBJPROP_COLOR, C'25,29,38'); // Еле заметный цвет
      ObjectSetInteger(0, wmName, OBJPROP_BACK, true);
      ObjectSetInteger(0, wmName, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, wmName, OBJPROP_ZORDER, -10);
   }
}
void UpdateLayout() {
...
bool BlockTerminal() {
   ...
   string name = "QPRO_BLOCK_MSG";
   if(ObjectFind(0, name) < 0) {
      ObjectCreate(0, name, OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(0, name, OBJPROP_CORNER, CORNER_LEFT_UPPER); // Привязка к левому верхнему углу
      ObjectSetString(0, name, OBJPROP_FONT, "Trebuchet MS");
      ObjectSetInteger(0, name, OBJPROP_FONTSIZE, 18);
      ObjectSetInteger(0, name, OBJPROP_COLOR, C'242,54,69');
      ObjectSetInteger(0, name, OBJPROP_ANCHOR, ANCHOR_CENTER);     // Текст будет расти из центра своих координат
      ObjectSetInteger(0, name, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(0, name, OBJPROP_BACK, false);
   ...
   ObjectSetInteger(0, name, OBJPROP_XDISTANCE, chart_width / 2);
   ObjectSetInteger(0, name, OBJPROP_YDISTANCE, chart_height / 2);
   
   ObjectSetString(0, name, OBJPROP_TEXT, "⛔️ 1-HOUR TRIAL EXPIRED ⛔️\nTo continue, upgrade to PRO on MQL5 Market");

   
   FileDelete("QPRO_Chart_" + current_symbol + ".csv");
